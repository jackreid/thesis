...
The chapter starts in Section \ref{sec:intended} by walking through the proposed \ac{evdt} Framework element by element. Section \ref{sec:intended} then presents the intended uses of the framework in more detail. This is followed by Section \ref{sec:novelty} which compares the proposed frameworks to other approaches and discusses the novelty of the proposed framework. Section \ref{sec:evdt-conclusion} finally recaps the framework and sets the stage for the subsequent two chapters, which present applied case studies of the \ac{evdt} Framework for those interested in concrete demonstrations. Chapter \ref{ch:discussion} will finally revisit the \ac{evdt} Framework in light of the two case studies and discuss plans for the future.
...
Each of these elements span the entire lifecycle of an \ac{evdt} project, but can still be usefully considered in the order listed. The following subsections walk through each of these elements. Later sections in this chapter address in more detail the intended uses of the framework and its novelty.
...
\caption[Generic System Context Dimensions]{Generic System Context Dimensions, including example topics to include.}
...
\subsection{\hlc[cyan]{Collaborative Development}} \label{sec:evdt-collab}
...
\begin{enumerate} \setlength{\itemsep}{0pt} \setlength{\parskip}{0pt}
    \item{\textbf{Pre-Project:} Ideation through the initial, tentative definitions of the System Boundary. An \ac{evdt} project should not be perceived without some active interest from one or more local stakeholder \footnote{See \textit{Local Context Expert} in Section \ref{sec:intended}.}.  The more local the better (i.e. a resident or member of a municipal government is to generally to be preferred over a national government minister). This initial contact should not be assumed to be the sole representative of the community but they are integral to starting to grapple with the system of interest.}
...
\subsection{\hlc[cyan]{EVDT Questions \& Models}} \label{sec:evdt-questions}
...
\subsection{\hlc[cyan]{Interactive Decision Support System}} 
...
\subsection{\hlc[cyan]{Re-use \& Community Development}} \label{sec:reuse}
...
\section{\hlc[cyan]{Novelty and Utility of EVDT}} \label{sec:novelty}
...
The \ac{evdt} Framework also certainly builds upon Jankowski's and Nyerges' macro-micro strategy for participatory decision-making. This strategy, shown in Table \ref{tab:macro-micro}, envisions decion-making as taking place across three macro-phases (Intelligence, Design, and Choice), each of which are made up of an iteration of the same four micro-activities (Gather, Organize, Select, and Review). The first iteration of the \ac{saf} is essentially a specific approach for accomplishing Phase 1A through 2A. The second iteration of \ac{saf} results in the creation of a \ac{dss} that in turn supports the community in accomplishing Phase 2B through 3D. 
